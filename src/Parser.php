...
class Parser extends OptionContainer
{
    ...
    public function __construct($args = array())
    {
        ...
        if (is_null($args['formatter'])) {
            $args['formatter'] = new IndentedHelpFormatter();
        }
    ...
    public static function matchAbbrev($s, $wordmap)
    {
        ...
        if (count($possibilities) == 1) {
            ...
            throw new BadOptionException($s);
        } else {
            // More than one possible completion: ambiguous prefix.
            sort($possibilities);
            throw new AmbiguousOptionException($s, $possibilities);
        }
    }
        ...
        if ($option->takesValue()) {
            ...
            if (count($rargs) < $nargs) {
                if (!$option->hasDefault()) {
                    if ($nargs == 1) {
                        $this->parserError(sprintf(Translation::t("%s option requires an argument"), $opt));
                    } else {
            ...
            $this->parserError(sprintf(Translation::t("%s option does not take a value"), $opt));
        ...
        for ($c = 1, $c_max = strlen($arg); $c < $c_max; $c++) {
            ...
            if (!$option) {
                if ($this->allowUnknownArgs) {
                    ...
                    throw new BadOptionException($opt);
                }
            }
